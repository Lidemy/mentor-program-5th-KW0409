...
<h3>什麼是 DNS?Google 有提供公開的 DNS，對 Google 的好處以及對一般大眾的好處是什麼？</h3>
...
<h4>Google 有提供公開的 DNS，對 Google 的好處以及對一般大眾的好處是什麼？</h4>
...
<h3>什麼是資料庫的 lock?為什麼我們需要 lock？</h3>
...
<h3>NoSQL 跟 SQL 的差別在哪裏？</h3>

NoSQL(Not only SQL)，是所有『非關聯式資料庫』的統稱
相較於使用 SQL(Srtuctured Query Language, 結構式查詢語言) 語法的關聯式資料庫，
**NoSQL 沒有固定的 Schema，並且是用 key-value 的形式來存資料，因此可以想像成是存像是 JSON 格式的資料進資料庫**(如下圖)，
(附圖來自 [這邊](https://codegym.tech/blog/sql_vs_nosql.html))
