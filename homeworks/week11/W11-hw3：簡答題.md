...
首先在使用上，`include()`、`require()`、`include_once()`、`require_once()`，都屬於可以直接飲用外部檔案的函式

1. **`include()`**：此函式會將指定的檔案讀入，並且執行裡面的程式
   例如： `include('./goodfile');`
   被匯入的檔案 goodfile 中的程式碼都會被執行，因此 `include()` 通常會被用來匯入動態的內容

2. **`require()`**：此函式會將指定的檔案讀入，並且將 `require()` 本身代換成導入的檔案裡面的程式
   例如： `require('./goodfile');`
   require() 只會將本身代換成被匯入的檔案 goodfile 中的程式碼而不會去執行，因此 `require()` 通常會被用來匯入動態的內容

3. **`include_once()`**：在使用上幾乎跟 `include()` 是相同的，區別就在於 `include_once()` 會先檢查要匯入的檔案是不是已經在此檔案中的其他地方被匯入過

4. **`require_once()`**：同樣的，在使用上幾乎跟 `require()` 是相同的，區別就在於 `require_once()` 會先檢查要匯入的檔案是不是已經在此檔案中的其他地方被匯入過

而 `include()` 與 `require()` 除了在處理引入的文件的方式上有所不同之外，最大的區別在於
`include()` 在引入不存在的文件時會產生一個警告但會繼續往下執行程式碼
`require()` 若是引入不存在的文件則不會產生警告且會停止執行程式碼
並且 `include()` 是有條件包含函數，但 `require()` 卻是無條件包含函數，如下

```php
(在此例中，如果 $condition 變數為 true，才會引入 goodfile)
...
```php
(在此例中，不管 $condition 變數為何，都會引入 goodfile)
