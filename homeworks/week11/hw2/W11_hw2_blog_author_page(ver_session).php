...
if (empty($_GET['author_name'])) {
  ...
  exit();
} else if (empty($_GET['author_nickname'])) {
  header('Location: W11_hw2_blog_index(ver_session).php');
  exit();
}
$author_name = $_GET['author_name'];
$author_nickname = $_GET['author_nickname'];
...
'WHERE A.username = ? AND is_deleted IS NULL ' .
'ORDER BY id DESC ' .
'LIMIT ? OFFSET ?';
$stmt = $conn->prepare($sql);
$stmt->bind_param('sii', $author_name, $num_per_page, $offset);
$result = $stmt->execute();
if (empty($result)) {
...
<body>
  <nav class="navbar">
    <div class="wrapper navbar__wrapper">
      <div class="navbar__site-name">
        <a href="W11_hw2_blog_author_page(ver_session).php?author_name=<?php echo $author_name?>&author_nickname=<?php echo escape($author_nickname) ?>">
          <?php echo escape($author_nickname) ?>'s Blog
        </a>
      </div>
      ...
      <ul class="navbar__list">
        <div>
          ...
          <li><a href="#">關於 <?php echo $author_nickname?>(暫未開放)</a></li>
        </div>
      ...
      <?php while ($data_arr = $result->fetch_assoc()) { ?>
        <div class="admin-posts">
          ...
          <div class="list__author-info">
            Posted by 
            <a class="list__author-link" href="W11_hw2_blog_author_page(ver_session).php?author_name=<?php echo escape($data_arr['username'])?>&author_nickname=<?php echo escape($author_nickname) ?>">
  ...
  <div class="page__btn-area">
    ...
    <?php if ($page !== 1) { ?>
      <a class="top__page-btn style1-btn" href="W11_hw2_blog_author_page(ver_session).php?author_name=<?php echo $author_name ?>&author_nickname=<?php echo escape($author_nickname) ?>&page=1">首頁</a>
      <a class="style1-btn style2-btn" href="W11_hw2_blog_author_page(ver_session).php?author_name=<?php echo $author_name ?>&author_nickname=<?php echo escape($author_nickname) ?>&page=<?php echo $page - 1 ?>">
        &larr; 上一頁
      </a>
    <?php } ?>
    ...
    <?php if ($page !== $total_page) { ?>
      ...
      <a class="end__page-btn style1-btn" href="W11_hw2_blog_author_page(ver_session).php?author_name=<?php echo $author_name ?>&author_nickname=<?php echo escape($author_nickname) ?>&page=<?php echo $total_page ?>">末頁</a>
    <?php } ?>
  </div>
